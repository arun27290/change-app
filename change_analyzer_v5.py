...
def analyse(df_raw):
    ...
    # ── Task assignee pivot: group → [{name, count, unique_changes}] ────────────
    task_assignee_pivot = {}   # {group_name: [{name, count, unique_changes}, ...]}
    if F["Task_Assignee_Group"] and "Task_Assignee_Group" in df_all.columns:
        # Build a per-assignee-group, per-assignee count from df_all
        _task_col = "Task_ID" if F["Task_ID"] else None
        for gname, gdf in df_all.groupby("Task_Assignee_Group", dropna=False):
            g = str(gname)
            # Each row IS a task (Task_ID is unique), so count rows per assignee group
            # There is no individual task-assignee name column — group IS the assignee unit
            # So we show sub-breakdown by Change_No for that group if available
            if F["Change_No"]:
                cr_counts = gdf["Change_No"].value_counts().head(10)
                members = [{"name": str(cr), "count": int(cnt)}
                           for cr, cnt in cr_counts.items()]
            else:
                members = []
            task_assignee_pivot[g] = {
                "total":          len(gdf),
                "unique_changes": len(gdf["Change_No"].dropna().unique()) if F["Change_No"] else 0,
                "top_changes":    members,
            }
...
// ── tasks ─────────────────────────────────────────────────────────────────────
let _taskPivot = {};       // { groupName: {total, unique_changes, top_changes:[{name,count}]} }
let _taskPivotGroups = []; // ordered group names
...
function renderTaskPivot(groupNames) {
  if (!groupNames.length) {
    $('tsk-pivot-list').innerHTML = '<div style="color:var(--muted);font-size:.82rem;padding:12px">No task assignee data available.</div>';
    return;
  }
  ...
  $('tsk-pivot-list').innerHTML = groupNames.map(gname => {
    const info = _taskPivot[gname] || {total:0, unique_changes:0, top_changes:[]};
    const grpPct = grandTotal ? info.total/grandTotal*100 : 0;
    const maxCRCount = info.top_changes.length ? Math.max(...info.top_changes.map(r=>r.count)) : 1;

    // Pivot table — rows are top Change_No values for this group
    const tableRows = info.top_changes.map(r => {
      const pct = maxCRCount ? r.count/maxCRCount*100 : 0;
      return `<tr>
        <td class="mono" style="color:var(--blue);font-weight:600;font-size:.8rem">${r.name}</td>
    ...
    return `<div class="grp-panel" id="${safeId}" style="margin-bottom:8px">
      <div class="grp-hdr" onclick="this.parentElement.classList.toggle('open')">
        <div style="flex:1;min-width:0">
          <div class="grp-name">${gname}</div>
          <div class="grp-meta">${fmt(info.total)} task${info.total!==1?'s':''} · across ${fmt(info.unique_changes)} change${info.unique_changes!==1?'s':''}</div>
      ...
      <div class="grp-body" style="padding:0">
        <div style="overflow-x:auto;max-height:320px;overflow-y:auto">
          <table style="width:100%">
            <thead>
              <tr>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf)">Change No (top tasks)</th>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf);text-align:right;white-space:nowrap"># Tasks</th>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf);width:140px">Share</th>
              </tr>
            </thead>
            <tbody>
              ${tableRows||`<tr><td colspan="3" style="color:var(--muted);font-size:.78rem;padding:10px">No change breakdown available.</td></tr>`}
            </tbody>
            <tfoot>
              <tr style="border-top:2px solid var(--border2)">
                <td style="font-weight:700;font-size:.8rem;color:var(--muted)">Total Tasks</td>
        ...
        <div style="padding:10px 14px;font-size:.75rem;color:var(--muted);border-top:1px solid var(--border)">
          Unique changes involving this group: <strong style="color:var(--blue)">${fmt(info.unique_changes)}</strong>
        </div>
      </div>
    </div>`;
  }).join('');

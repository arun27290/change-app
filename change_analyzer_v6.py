...
CANONICAL = {
    ...
    "Change_Description":           ["Change_Description","Change Description","ChangeDesc","Change Desc","CR Description","CR Desc","Change Summary"],
    "Change_Coordinator":           ["Change_Coordinator","Change Coordinator","ChangeCoordinator","CR Coordinator","CR Owner","Change Owner","Change Manager"],
    ...
    "ChangeStatus":                 ["ChangeStatus","Change Status","Change_Status","CR Status","CR State","Change State"],
    "Scheduled_Start_Date":         ["Scheduled Start Date","Scheduled_Start_Date","ScheduledStartDate","CR Start Date","CR Sched Start","Change Start Date","Planned Start"],
    "Scheduled_End_Date":           ["Scheduled End Date","Scheduled_End_Date","ScheduledEndDate","CR End Date","CR Sched End","Change End Date","Planned End"],
    "Task_ID":                      ["Task_ID","Task ID","TaskID","Task No","TaskNo","Task Number","Task_No"],
    "Task_Summary":                 ["Task_Summary","Task Summary","TaskSummary","Task Desc","Task Description","Task_Desc","Task_Description","Summary of Task","Task Details"],
    "Task_Status":                  ["Task_Status","Task Status","TaskStatus","Task State","TaskState","Task_State"],
    "Task_Assignee":                ["Task_Assignee","Task Assignee","TaskAssignee","Assignee","Implementor","Task Implementor","Task_Implementor","Implemented By","Task Owner","Task Assigned To"],
    "Task_Scheduled_Start_Date":    ["Task_Scheduled_Start_Date","Task Scheduled Start Date","TaskScheduledStartDate","Task Sched Start","Task Planned Start"],
    "Task_Scheduled_End_Date":      ["Task_Scheduled_End_Date","Task Scheduled End Date","TaskScheduledEndDate","Task Sched End","Task Planned End"],
    ...
    "Task_Assignee_Group":          ["Task_Assignee_Group","Task Assignee Group","TaskAssigneeGroup","Task Group","Task Assigned Group","Task Team","Assignee Group"],
}
...
def _squeeze_col(df, name):
    """Always return a 1-D Series for df[name], even if duplicate columns exist."""
    col = df[name]
...
def normalise_columns(df):
    ...
    # Step 2: drop duplicate raw column names BEFORE rename (keep first)
    if df.columns.duplicated().any():
        raw_dupes = df.columns[df.columns.duplicated()].unique().tolist()
        log.warning("Duplicate raw columns (keeping first): %s", raw_dupes)
        df = df.loc[:, ~df.columns.duplicated(keep="first")]
    # Step 3: fuzzy-map remaining columns to canonical names
    ...
    # Step 4: deduplicate again after rename (two raw cols → same canonical name)
    if df.columns.duplicated().any():
        post_dupes = df.columns[df.columns.duplicated()].unique().tolist()
        log.warning("Duplicate columns after mapping (keeping first): %s", post_dupes)
        df = df.loc[:, ~df.columns.duplicated(keep="first")]
    return df
...
def _safe_parse_one(v):
    """Parse a single cell value to datetime — never raises."""
    ...
    s = str(v).strip()
    if s in ("", "nan", "NaT", "None", "NaN", "nat"):
        return pd.NaT
    # Try Excel serial number
    try:
        f = float(s)
        if f > 0:
            return pd.Timestamp(EXCEL_EPOCH + timedelta(days=f))
    except (ValueError, TypeError):
        pass
    # Try dd/mm/yyyy hh:mm:ss  (your format)
...
def parse_date_col(series):
    """Robustly parse any date column to datetime64 — handles mixed types,
    non-contiguous index, blank strings, Excel serials, and dd/mm/yyyy."""
    if pd.api.types.is_datetime64_any_dtype(series):
        return series
    # Work on a plain Python list to avoid ALL pandas index/dtype issues
    orig_index = series.index
    values = list(series)
    parsed_vals = [_safe_parse_one(v) for v in values]
    result = pd.Series(pd.to_datetime(parsed_vals, errors="coerce"), index=orig_index)
    return result
...
def analyse(df_raw):
    ...
    # ── Parse date columns ─────────────────────────────────────────────────────
    # Squeeze any accidentally 2-D column slices to 1-D Series first,
    # then pass through the bulletproof cell-by-cell parser.
    for dcol in ["Scheduled_Start_Date","Scheduled_End_Date",
        ...
        if dcol in df_full.columns:
            df_full[dcol] = parse_date_col(_squeeze_col(df_full, dcol))
    ...
    status_data = {}
    if F["ChangeStatus"]:
        _cs = _squeeze_col(df_cr, "ChangeStatus").fillna("Unknown").astype(str)
        df_cr["ChangeStatus"] = _cs
        vc = _cs.value_counts()
        status_data = {str(k): int(v) for k, v in vc.items()}
        closed_ct = int(_cs.str.strip().str.lower().isin(CLOSED_CHANGE_STATUSES).sum())
        open_ct   = int(_cs.str.strip().str.lower().isin(OPEN_CHANGE_STATUSES).sum())
    else:
        closed_ct = open_ct = 0
    ...
    if F["CR_Assignee_Group"]:
        crg = _squeeze_col(df_cr, "CR_Assignee_Group").fillna("").astype(str).str.strip()
    ...
    if F["Change_Coordinator"]:
        cc = _squeeze_col(df_cr, "Change_Coordinator").fillna("").astype(str).str.strip()
    ...
    # ── Coordinator pivot: group → [{name, count}] sorted by count desc ─────────
    coordinator_pivot = {}   # {group_name: [{name, count}, ...]}
    if F["Change_Coordinator"] and F["CR_Assignee_Group"]:
        for row in coordinator_rows:
            g = row["group"]
            if g not in coordinator_pivot:
                coordinator_pivot[g] = []
            coordinator_pivot[g].append({"name": row["name"], "count": row["total"]})
        # Each group list is already sorted desc (coordinator_rows sorted by total)
        # Re-sort per group just to be sure
        for g in coordinator_pivot:
            coordinator_pivot[g].sort(key=lambda x: x["count"], reverse=True)
    elif F["Change_Coordinator"]:
        # No group column — put everyone under a single "(All)" bucket
        coordinator_pivot["(All)"] = [{"name": r["name"], "count": r["total"]}
                                       for r in coordinator_rows]

    # ── Normalise new task text columns ──────────────────────────────────────
    for _tc in ["Task_Status", "Task_Assignee", "Task_Summary"]:
        if F.get(_tc) and _tc in df_all.columns:
            _s = _squeeze_col(df_all, _tc).fillna("").astype(str).str.strip()
            df_all[_tc] = _s.where(_s != "", "(Blank)")

    # ── Task Status distribution ──────────────────────────────────────────────
    task_status_data = {}
    if F.get("Task_Status") and "Task_Status" in df_all.columns:
        task_status_data = {str(k): int(v)
                            for k, v in df_all["Task_Status"].value_counts().items()}

    # ── Task Assignee Group analysis ──────────────────────────────────────────
    task_group_rows = []
    if F["Task_Assignee_Group"]:
        # Fill NaN and whitespace-only task groups (common for cancelled tasks)
        tg = _squeeze_col(df_all, "Task_Assignee_Group").fillna("").astype(str).str.strip()
        df_all["Task_Assignee_Group"] = tg.where(tg != "", "(Blank / Cancelled)")
        for gname, gdf in df_all.groupby("Task_Assignee_Group", dropna=False):
            cr_nos = (gdf["Change_No"].dropna().unique().tolist()
                      if F["Change_No"] else [])
            # assignees in this group
            assignees = []
            if F.get("Task_Assignee") and "Task_Assignee" in gdf.columns:
                assignees = sorted(set(
                    str(a) for a in _squeeze_col(gdf, "Task_Assignee").dropna()
                    if str(a) not in ("", "(Blank)")
                ))
            # status breakdown
            statuses = {}
            if F.get("Task_Status") and "Task_Status" in gdf.columns:
                statuses = {str(k): int(v)
                            for k, v in _squeeze_col(gdf, "Task_Status")
                            .value_counts().items()}
    ...
    # ── Task assignee pivot: group → [{name, count, statuses}] ───────────────
    # Now that Task_Assignee column exists, show real assignee names per group.
    # Falls back to top Change_No breakdown if Task_Assignee not present.
    task_assignee_pivot = {}
    if F["Task_Assignee_Group"] and "Task_Assignee_Group" in df_all.columns:
        for gname, gdf in df_all.groupby("Task_Assignee_Group", dropna=False):
            g = str(gname)
            if F.get("Task_Assignee") and "Task_Assignee" in gdf.columns:
                # Pivot: assignee name → task count + status breakdown
                members = []
                for aname, adf in gdf.groupby(
                        _squeeze_col(gdf, "Task_Assignee").fillna("(Blank)").values,
                        sort=False):
                    aname = str(aname)
                    # per-assignee status breakdown
                    ast = {}
                    if F.get("Task_Status") and "Task_Status" in adf.columns:
                        ast = {str(k): int(v)
                               for k, v in _squeeze_col(adf, "Task_Status")
                               .value_counts().items()}
                    members.append({"name": aname, "count": len(adf), "statuses": ast})
                members.sort(key=lambda x: x["count"], reverse=True)
            else:
                # Fallback: show top Change_No for this group
                if F["Change_No"]:
                    cr_counts = _squeeze_col(gdf, "Change_No").value_counts().head(10)
                    members = [{"name": str(cr), "count": int(cnt), "statuses": {}}
                               for cr, cnt in cr_counts.items()]
                else:
                    members = []
            task_assignee_pivot[g] = {
                "total":          len(gdf),
                "unique_changes": (len(_squeeze_col(gdf, "Change_No").dropna().unique())
                                   if F["Change_No"] else 0),
                "members":        members,
                "has_assignee":   bool(F.get("Task_Assignee")),
            }
    ...
    if len(months_sorted) >= 2:
        ...
        def _status_ct(month, status_set):
            ...
            return int(_squeeze_col(sub, "ChangeStatus").astype(str).str.strip().str.lower().isin(status_set).sum())
    ...
    def _gen_charts(theme):
        ...
        # Task actual completion fill rate donut
        ...
        if task_status_data:
            tsl = list(task_status_data.keys()); tsv = list(task_status_data.values())
            tsc = [status_color(l) for l in tsl]
            c["task_status_donut"] = make_donut(
                tsl, tsv, colors=tsc, title="Task Status Distribution")
    ...
    return {
        ...
        "coordinator_pivot":  coordinator_pivot,
        "task_group_rows":    task_group_rows,
        "task_assignee_pivot": task_assignee_pivot,
...
<div id="dash">
  ...
  <div class="tab-pane" id="pane-chg">
    <div class="tab-body">
      <div class="sec-hdr">Coordinator Pivot — Changes by Group</div>
      <input class="search-box" id="chg-search" placeholder="Search group or coordinator…" oninput="filterCoordPivot()"/>
      <!-- scrollable accordion list -->
      <div id="chg-pivot-list" style="max-height:520px;overflow-y:auto;padding-right:4px"></div>

      <div class="sec-hdr" style="margin-top:28px">Top Changes by Task Count</div>
      <div class="tbl-wrap">
        <table>
          <thead>
  ...
  <div class="tab-pane" id="pane-tsk">
    <div class="tab-body">
      <div class="sec-hdr">Task Assignee Pivot — Tasks by Group</div>
      <input class="search-box" id="tsk-search" placeholder="Search group…" oninput="filterTaskPivot()"/>
      <!-- scrollable accordion list -->
      <div id="tsk-pivot-list" style="max-height:520px;overflow-y:auto;padding-right:4px"></div>
...
// ── changes ───────────────────────────────────────────────────────────────────
let _coordPivot = {};   // { groupName: [{name, count}, ...] }
let _coordPivotGroups = [];  // ordered group names

function buildChanges(d) {
  _coordPivot = d.coordinator_pivot || {};
  _coordPivotGroups = Object.keys(_coordPivot);
  renderCoordPivot(_coordPivotGroups);
...
function renderCoordPivot(groupNames) {
  if (!groupNames.length) {
    $('chg-pivot-list').innerHTML = '<div style="color:var(--muted);font-size:.82rem;padding:12px">No coordinator data available.</div>';
    return;
  }
  // Grand total across all groups (for % bar scaling)
  const grandTotal = groupNames.reduce((s,g)=>{
    return s + (_coordPivot[g]||[]).reduce((ss,r)=>ss+r.count,0);
  },0);

  $('chg-pivot-list').innerHTML = groupNames.map(gname => {
    const members = _coordPivot[gname] || [];
    const grpTotal = members.reduce((s,r)=>s+r.count,0);
    const grpPct   = grandTotal ? grpTotal/grandTotal*100 : 0;
    const maxCount = members.length ? Math.max(...members.map(r=>r.count)) : 1;

    // Pivot table rows for this group
    const tableRows = members.map(r => {
      const pct = maxCount ? r.count/maxCount*100 : 0;
      return `<tr>
        <td style="font-weight:600;font-size:.82rem">${r.name}</td>
        <td class="mono" style="color:var(--blue);font-weight:700;text-align:right;white-space:nowrap">${fmt(r.count)}</td>
        <td style="width:140px;padding-left:10px">
          <div class="mbar"><div class="mbar-fill" style="width:${pct.toFixed(1)}%;background:var(--blue)"></div></div>
        </td>
      </tr>`;
    }).join('');

    const safeId = 'cpvt_' + gname.replace(/[^a-zA-Z0-9]/g,'_');
    return `<div class="grp-panel" id="${safeId}" style="margin-bottom:8px">
      <div class="grp-hdr" onclick="this.parentElement.classList.toggle('open')">
        <div style="flex:1;min-width:0">
          <div class="grp-name">${gname}</div>
          <div class="grp-meta">${fmt(grpTotal)} change${grpTotal!==1?'s':''} · ${members.length} coordinator${members.length!==1?'s':''}</div>
          ${mbar(grpPct,'var(--blue)')}
        </div>
        <span class="chip c-blue" style="margin-right:10px;flex-shrink:0">${fmt(grpTotal)}</span>
        <span class="g-expand">▼</span>
      </div>
      <div class="grp-body" style="padding:0">
        <div style="overflow-x:auto;max-height:320px;overflow-y:auto">
          <table style="width:100%">
            <thead>
              <tr>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf)">Coordinator Name</th>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf);text-align:right;white-space:nowrap"># Changes</th>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf);width:140px">Share</th>
              </tr>
            </thead>
            <tbody>${tableRows}</tbody>
            <tfoot>
              <tr style="border-top:2px solid var(--border2)">
                <td style="font-weight:700;font-size:.8rem;color:var(--muted)">Total</td>
                <td class="mono" style="font-weight:800;color:var(--blue);text-align:right">${fmt(grpTotal)}</td>
                <td></td>
              </tr>
            </tfoot>
          </table>
...
function filterCoordPivot() {
  const q = $('chg-search').value.toLowerCase().trim();
  if (!q) { renderCoordPivot(_coordPivotGroups); return; }
  const filtered = _coordPivotGroups.filter(g => {
    if (g.toLowerCase().includes(q)) return true;
    return (_coordPivot[g]||[]).some(r => r.name.toLowerCase().includes(q));
  });
  renderCoordPivot(filtered);
}

// ── tasks ─────────────────────────────────────────────────────────────────────
let _taskPivot = {};
let _taskPivotGroups = [];

function buildTasks(d) {
  _taskPivot = d.task_assignee_pivot || {};
  _taskPivotGroups = Object.keys(_taskPivot);
  renderTaskPivot(_taskPivotGroups);
  $('tsk-charts').innerHTML =
    chartBox('task_status_donut') +
    chartBox('tskgrp_bar') +
    chartBox('task_fill');
}

function renderTaskPivot(groupNames) {
  if (!groupNames.length) {
    $('tsk-pivot-list').innerHTML =
      '<div style="color:var(--muted);font-size:.82rem;padding:12px">No task assignee data available.</div>';
    return;
  }
  const grandTotal = groupNames.reduce((s,g)=>s+(_taskPivot[g]?_taskPivot[g].total:0),0);

  $('tsk-pivot-list').innerHTML = groupNames.map(gname => {
    const info      = _taskPivot[gname] || {total:0,unique_changes:0,members:[],has_assignee:false};
    const grpPct    = grandTotal ? info.total/grandTotal*100 : 0;
    const members   = info.members || [];
    const maxCount  = members.length ? Math.max(...members.map(r=>r.count)) : 1;
    const colHdr    = info.has_assignee ? 'Task Assignee' : 'Change No';
    const colColor  = info.has_assignee ? 'var(--text)' : 'var(--blue)';

    const tableRows = members.map(r => {
      const pct = maxCount ? r.count/maxCount*100 : 0;
      // Status chips for this assignee
      const stChips = Object.entries(r.statuses||{}).map(([s,n])=>
        `<span style="margin-right:4px">${statusChip(s)}<span class="mono" style="font-size:.68rem;margin-left:2px">${n}</span></span>`
      ).join('');
      return `<tr>
        <td style="font-weight:600;font-size:.82rem;color:${colColor}">${r.name}</td>
        <td class="mono" style="font-weight:700;text-align:right;white-space:nowrap;color:var(--cyan)">${fmt(r.count)}</td>
        <td style="width:120px;padding-left:8px">
          <div class="mbar"><div class="mbar-fill" style="width:${pct.toFixed(1)}%;background:var(--cyan)"></div></div>
        </td>
        <td style="font-size:.72rem;min-width:120px">${stChips||'—'}</td>
      </tr>`;
    }).join('');

    const safeId = 'tpvt_' + gname.replace(/[^a-zA-Z0-9]/g,'_');
    return `<div class="grp-panel" id="${safeId}" style="margin-bottom:8px">
      <div class="grp-hdr" onclick="this.parentElement.classList.toggle('open')">
        <div style="flex:1;min-width:0">
          <div class="grp-name">${gname}</div>
          <div class="grp-meta">
            ${fmt(info.total)} task${info.total!==1?'s':''}
            · ${fmt(info.unique_changes)} change${info.unique_changes!==1?'s':''}
            ${info.has_assignee ? '· '+fmt(members.length)+' assignee'+(members.length!==1?'s':'') : ''}
          </div>
          ${mbar(grpPct,'var(--cyan)')}
        </div>
        <span class="chip c-cyan" style="margin-right:10px;flex-shrink:0">${fmt(info.total)}</span>
        <span class="g-expand">▼</span>
      </div>
      <div class="grp-body" style="padding:0">
        <div style="overflow-x:auto;max-height:340px;overflow-y:auto">
          <table style="width:100%">
            <thead>
              <tr>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf)">${colHdr}</th>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf);text-align:right"># Tasks</th>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf);width:120px">Share</th>
                <th style="position:sticky;top:0;z-index:2;background:var(--surf)">Task Status Breakdown</th>
              </tr>
            </thead>
            <tbody>
              ${tableRows||`<tr><td colspan="4" style="color:var(--muted);font-size:.78rem;padding:10px">No breakdown available.</td></tr>`}
            </tbody>
            <tfoot>
              <tr style="border-top:2px solid var(--border2)">
                <td style="font-weight:700;font-size:.8rem;color:var(--muted)">Total</td>
                <td class="mono" style="font-weight:800;color:var(--cyan);text-align:right">${fmt(info.total)}</td>
                <td colspan="2"></td>
              </tr>
            </tfoot>
          </table>
        </div>
        <div style="padding:8px 14px;font-size:.75rem;color:var(--muted);border-top:1px solid var(--border)">
          Unique changes: <strong style="color:var(--blue)">${fmt(info.unique_changes)}</strong>
        </div>
      </div>
    </div>`;
  }).join('');
...
function filterTaskPivot() {
  const q = $('tsk-search').value.toLowerCase().trim();
  if (!q) { renderTaskPivot(_taskPivotGroups); return; }
  renderTaskPivot(_taskPivotGroups.filter(g => g.toLowerCase().includes(q)));
}

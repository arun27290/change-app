...
def parse_date_col(series):
    ...
    # Try dayfirst=True since format is dd/mm/yyyy
...
def analyse(df_raw):
    ...
    if F["Change_No"]:
        ...
        df_cr = df_full.copy(); dedup_dropped = 0
    ...
    if date_ref_cr:
        # Use where() so NaT rows become "Unknown" instead of float NaN
        _per = df_cr[date_ref_cr].dt.to_period("M")
        df_cr["Month"]      = _per.astype(str).where(df_cr[date_ref_cr].notna(), "Unknown")
        df_cr["MonthLabel"] = df_cr[date_ref_cr].dt.strftime("%b %Y").where(df_cr[date_ref_cr].notna(), "Unknown")
        df_cr["Year"]       = df_cr[date_ref_cr].dt.year.astype("Int64").astype(str).where(df_cr[date_ref_cr].notna(), "Unknown")
    else:
    ...
    if date_ref_task:
        _tper = df_all[date_ref_task].dt.to_period("M")
        df_all["TaskMonth"]      = _tper.astype(str).where(df_all[date_ref_task].notna(), "Unknown")
        df_all["TaskMonthLabel"] = df_all[date_ref_task].dt.strftime("%b %Y").where(df_all[date_ref_task].notna(), "Unknown")
    else:
    ...
    months_sorted = sorted([
        ...
        if m not in (None, "Unknown") and str(m) not in ("nan", "NaT", "Unknown")
    ])
    monthly_cr_counts = []
    monthly_cr_labels = []
    for m in months_sorted:
        ...
        try:
            lbl = pd.Period(m, "M").strftime("%b %Y")
        except Exception:
            lbl = m
    ...
    if F["CR_Assignee_Group"]:
        df_cr["CR_Assignee_Group"] = df_cr["CR_Assignee_Group"].fillna("(Unassigned)").astype(str)
    ...
    if F["Change_Coordinator"]:
        df_cr["Change_Coordinator"] = df_cr["Change_Coordinator"].fillna("(Unknown)").astype(str)
    ...
    if F["Task_Assignee_Group"]:
        df_all["Task_Assignee_Group"] = df_all["Task_Assignee_Group"].fillna("(Unassigned)").astype(str)
    ...
    if len(months_sorted) >= 2:
        ...
        try:
            cur_lbl  = pd.Period(cur_m,  "M").strftime("%b %Y")
            prev_lbl = pd.Period(prev_m, "M").strftime("%b %Y")
        except Exception:
    ...
    for m in all_months:
        try: lbl = pd.Period(m,"M").strftime("%b %Y")
        except Exception: lbl = m
